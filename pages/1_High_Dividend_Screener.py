"""
High Dividend Stock Screener (Korean Version)
...
st.set_page_config(page_title="고배당 스크리너", page_icon="📊", layout="wide")

st.title("📊 고배당 종목 스크리너")
st.markdown("높은 배당 수익률의 종목을 필터링하고 분석합니다.")
...
min_yield = st.sidebar.slider(
    ...
    value=3.5,
    step=0.1,
    help="이 값 이상의 배당 수익률을 가진 종목만 필터링"
)
...
st.markdown("지표별 가중치 조정 (합계 1.0)")

col1, col2, col3, col4, col5 = st.columns(5)

with col1:
    w_yield = st.number_input("배당률", min_value=0.0, max_value=1.0, value=0.5, step=0.05)
with col2:
    w_years = st.number_input("연수", min_value=0.0, max_value=1.0, value=0.2, step=0.05)
with col3:
    w_cagr = st.number_input("5년 CAGR", min_value=0.0, max_value=1.0, value=0.1, step=0.05)
with col4:
    w_growth = st.number_input("1년 성장률", min_value=0.0, max_value=1.0, value=0.1, step=0.05)
with col5:
    w_payout = st.number_input("배당성향", min_value=0.0, max_value=1.0, value=0.1, step=0.05)
...
if len(filtered_df) > 0:
    ...
    if 'Div. Yield' in filtered_df.columns and 'Years' in filtered_df.columns:
        st.subheader("배당률 vs 연수 (버블 크기 = 점수)")
        fig3 = create_scatter_plot(
            filtered_df.head(50),
            x_col='Years',
            y_col='Div. Yield',
            size_col='high_div_composite',
            title="배당률 vs 배당 지급 연수",
